...
class ProviderChunk extends Model {
    ...
    getData() {
        ...
        const chunk_path = ProviderChunk.getChunkStoragePath(this.id);
        if (fs.existsSync(chunk_path)) {
            ...
            for (const segment_hash of this.segment_hashes) {
                const segment_path = ProviderChunk.getSegmentStoragePath(segment_hash, this.id);
                const buffer = fs.readFileSync(segment_path, {encoding: null}); // todo: what if doesn't exist? then we have an error: Error: ENOENT: no such file or directory, open '/Users/username/.point/test1/data/provider_storage_cache/provider_chunk_segment_a9bf0cd755c9058...
                data = Buffer.concat([data, buffer]);
                const segment_real_hash = utils.hashFnHex(buffer);
                if (segment_hash !== segment_real_hash)
                    throw new Error('EINVALIDHASH: Segment read from disk doesn\'t match its ID'); // todo: intercept?
            }
    ...
    hasDecryptedData() {
        // todo: what if file exists but has just been created and partially filled with decrypted data? (still being decrypted)
        const decryptedFileName = ProviderChunk.getDecryptedChunkStoragePath(this.id);
        return fs.existsSync(decryptedFileName);
    }

    validateSegmentHashes() {
        const merkleTree = utils.merkle.merkle(
            this.segment_hashes.map(x => Buffer.from(x, 'hex')),
            utils.hashFn
        );
    ...
    setSegmentData(rawData, segment_index) {
        ...
        if (!Buffer.isBuffer(rawData))
            throw Error('ProviderChunk.setSegmentData: data must be a Buffer!');
        const data_hash = utils.hashFnHex(rawData);
        if (data_hash !== segment_hash) {
            throw Error(
                'EINVALIDHASH: segment hash and data hash don\'t match: ' +
                    segment_hash +
                    ' vs ' +
                    data_hash
            );
...
ProviderChunk.init(
    ...
    {indexes: [{fields: ['status']}, {fields: ['real_id']}]}
);
