...
const networkPrivateKey = Buffer.from(
    new Web3().eth.accounts.create().privateKey.replace(/^0x/, ''),
    'hex'
);
...
node.authenticate = node.plugin(
    AuthenticatePlugin({config, utils, log: console}, networkPublicKey, networkPrivateKey)
);

const targetUrl = `${protocol}//localhost:${config.network.communication_port}/#${ethUtil
    .privateToAddress(Buffer.from(config.client.wallet.privateKey, 'hex'))
    .toString('hex')}`;
