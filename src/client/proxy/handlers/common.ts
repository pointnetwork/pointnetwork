...
const getHttpRequestHandler = () => async (req: FastifyRequest, res: FastifyReply) => {
    try {
        ...
        if (!host) {
            ...
            const configZappsDir: string = config.has('zappsdir') ? String(config.get('zappsdir')) : '';
            let zappDir: string;
            if (configZappsDir) {
            ...
            if (!existsSync(publicPath)) throw new Error('Public path ' + publicPath + ' doesnt exist.');

            const routesJsonPath = path.resolve(zappDir, 'routes.json');
            if (!existsSync(routesJsonPath)) throw new Error('Routes file ' + routesJsonPath + ' doesnt exist.');
