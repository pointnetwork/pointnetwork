...
task("identity-add-deployer", "Will add a deployer to an identity")
  .addPositionalParam("address","Identity contract source address")
  .addPositionalParam("identity", "Identity to add the deployer")
  .addPositionalParam("deployer", "Address of the deployer to be added")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    try{
        await identityContract.connect(owner).addIdentityDeployer(taskArgs.identity, taskArgs.deployer);
    }catch (e){
        console.log('error while adding identity deployer');
        throw e;
    }
    console.log('Deployer added to the identity');
  });
