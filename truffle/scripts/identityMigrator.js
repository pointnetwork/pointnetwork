...
const migrationData = [
    {
        ...
        ikv: {
            ...
            'zweb/contracts/abi/Twitter':
                '03575fe9938f7bb2160d998eee3fecf3452f99917fe37e07189c7bf95bb04947',
...
async function main() {
    ...
    const migrationsApplied = await contract.methods.migrationApplied
    ...
    if (!migrationsApplied) {
        try {
            for (const i in migrationData) {
                const identity = migrationData[i];
                ...
                if ('ikv' in identity) {
                    for (const key in identity.ikv) {
                        const value = identity.ikv[key];

                        console.log(
                            `Setting key-value data identity:@${identity.handle} > [key:${key} --- value:${value}]`
                        );

                        await contract.methods
                            .ikvMigrate(identity.handle, key, value)
                            .send({from: accounts[0]});
                    }
                } else {
