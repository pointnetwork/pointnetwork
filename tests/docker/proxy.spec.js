...
describe('Proxy', () => {
    it('Should redirect from http://point to https://point', async () => {
        ...
        const res = await get('http://point', {
            httpAgent,
            maxRedirects: 0,
            validateStatus: () => true
        });
    ...
    it('Should return https://point HTML', async () => {
        ...
        const res = await get('https://point', {httpsAgent});
    ...
    it(
        ...
        async () => {
            ...
            await delay(5000);
            const res = await get('https://blog.point/notexists', {
                httpsAgent,
                validateStatus: () => true
            });
    ...
    it('Should return 404 for host other than point and not ending on .point', async () => {
        ...
        const res = await get('https://something.net', {
            httpsAgent,
            validateStatus: () => true
        });
        expect(res.status).toEqual(404);
    ...
    it('Should return 404 for host non-existing .point domain', async () => {
        ...
        const res = await get('https://notexists.point', {
            httpsAgent,
            validateStatus: () => true
        });
        expect(res.status).toEqual(404);
